...
describe('SecretCheck', () => {
  ...
  it('should detect API keys', async () => {
    writeFileSync(join(tempDir, 'test.js'), 'const apiKey = "sk-1234567890abcdef1234567890abcdef";');
    
    const result = await secretCheck.run(context);
    
    expect(result.passed).toBe(false);
    expect(result.details?.findings).toHaveLength(1);
    expect(result.details?.findings[0].type).toBe('API Key');
  });

  it('should detect JWT tokens', async () => {
    writeFileSync(join(tempDir, 'test.js'), 'const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c";');
    
    const result = await secretCheck.run(context);
    
    expect(result.passed).toBe(false);
    expect(result.details?.findings).toHaveLength(1);
    expect(result.details?.findings[0].type).toBe('JWT Token');
  });
  ...
  it('should detect AWS keys', async () => {
    ...
    expect(result.passed).toBe(false);
    expect(result.details?.findings).toHaveLength(1);
    expect(result.details?.findings[0].type).toBe('AWS Key');
  });

  it('should detect private keys', async () => {
    writeFileSync(join(tempDir, 'test.js'), 'const privateKey = "-----BEGIN PRIVATE KEY-----\\nMIIEvQIBADANBgkqhkiG9w0BAQEFAASCBKcwggSjAgEAAoIBAQC7VJTUt9Us8cKB...";');
    
    const result = await secretCheck.run(context);
    
    expect(result.passed).toBe(false);
    expect(result.details?.findings).toHaveLength(1);
    expect(result.details?.findings[0].type).toBe('Private Key');
  });

  it('should detect database URLs', async () => {
    writeFileSync(join(tempDir, 'test.js'), 'const dbUrl = "postgres://user:password@localhost:5432/mydb";');
  ...
  it('should detect generic secrets', async () => {
    ...
    expect(result.passed).toBe(false);
    expect(result.details?.findings).toHaveLength(1);
    expect(result.details?.findings[0].type).toBe('Generic Secret');
  });

  it('should provide file and line information', async () => {
    writeFileSync(join(tempDir, 'test.js'), 'console.log("line 1");\nconst apiKey = "sk-1234567890abcdef";\nconsole.log("line 3");');
    
    const result = await secretCheck.run(context);
    
    expect(result.details?.findings[0].line).toBe(2);
    expect(result.details?.findings[0].file).toBe('/test.js');
  });
